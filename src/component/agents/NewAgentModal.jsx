...
const NewAgentModal = ({ toggle }) => {
  ...
  const handleSubmit = async () => {
    ...
    // const allagentemail = Agentdata.map((i)=>{
    //   return i.email
    ...
    if (!formData.contact_number) {
      ...
      formErrors.contact_number = "Contact number must be 10 digits";
    }
    if (!formData.email) {
      ...
      Agentdata.some((existingUser) => existingUser.email === formData.email)
    ) {
      formErrors.email = "This email already exists. Please use another email";
    }
    ...
    if (!formData.commission_percentage)
      formErrors.commission_percentage = "Commission % is required";
