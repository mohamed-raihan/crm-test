...
import NewPaymentModal from './Newuserpayment.jsx';  // Adjusted import for payment modal
import EditPaymentModal from './Editpayment.jsx';
...
const PaymentTable = () => {
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      // Filter payments where created_by matches the logged-in user's ID
      ...
      console.error("Error occurred while fetching or processing payment data:", error);
    }
  };
...
console.log('customer',Customerdata);
  ...
  const handleDeletePayment = async (id) => {
    try {
      const response = await allaxios.delete(API_URL.PAYMENT.DELETE_PAYMENT(id), {
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem('authToken')}`,
        },
      });
  ...
  const handleSearch = (e) => {
    ...
    //   payment.policy_name.toLowerCase().includes(value) || payment.customer_name.toLowerCase().includes(value)
    // );
    ...
    <div className="table-wrapper">
      <div className='d-flex flex-wrap justify-content-between p-3 border mb-1 shadow position-relative'>
        ...
        <div className='d-flex'>
          ...
          />
          <Button onClick={toggle} className='btn bg-primary'>+ Add New Payment</Button>
        </div>
      </div>
      <DataTable
        className="border shadow"
        data={paymentData}
        columns={BranchPaymentTableColumn(handleEditPayment, handleDeletePayment,Customerdata,Policydata)}  // Adjusted to handle payment data
        customStyles={{
          table: {
            style: {
              width: "100%",
